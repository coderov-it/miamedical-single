 ...
 * wrote the first time. Where a file declares a LIST, the list is reconciled:
 * rows the file no longer contains are deleted. That is what makes the JSON the
 * source of truth rather than an append-only feed — deleting a spec from a file
 * has to delete it from the catalogue, or the two drift apart with no way back.
 *
...
import { richTextToPlain, sanitizeRichText } from '../../src/shared/html/rich-text.ts';
import type {
  CatalogPlan,
  PlannedCategory,
  PlannedProduct,
