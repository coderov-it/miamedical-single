...
export function customerInfoIt(customer: ContractCustomer): string {
  ...
  if (customer.codiceFiscale) {
    fiscalFields.push(
      `<div class="info-block"><label>Codice Fiscale</label><span>${escapeHtml(customer.codiceFiscale)}</span></div>`,
    );
  }
  if (customer.partitaIva) {
    fiscalFields.push(
      `<div class="info-block"><label>Partita IVA</label><span>${escapeHtml(customer.partitaIva)}</span></div>`,
    );
  }
  if (customer.codiceUnivoco) {
    fiscalFields.push(
      `<div class="info-block"><label>Codice Univoco</label><span>${escapeHtml(customer.codiceUnivoco)}</span></div>`,
    );
  }
