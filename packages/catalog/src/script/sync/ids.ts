...
/**
 * Every primary key the sync writes, derived from the `code` a data file
 * declares rather than handed out by `defaultRandom()`.
 *
 ...
 *
 * Keyed on the hand-written `code`, never on an id from anywhere else — which
 * is what lets a data file be renamed, reordered or rewritten without
...
export const termsId = (code: string): string => uuidV5(`terms:${code}`);
